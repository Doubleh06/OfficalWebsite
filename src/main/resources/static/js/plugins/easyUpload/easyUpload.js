    ...
    this.each(function (index, element) {
      ...
      var easyManager = {
        init: function () {
          ...
          $html += option.multi ? '<input type="checkbox" class="easyUploadIcon noselect head_check" data-checked="no"/>' : '';
          ...
          _ele.html($html);
            //初始化查看是否有图片需要回显
            if(""!=defaults.callbackImg&&null!=defaults.callbackImg){
                var data = defaults.callbackImg.split("|");
                var $li = "";
                for(var i=0;i<data.length;i++){
                    var preview;
                    var fileType = data[i].split("~")[1].split('.').pop();
                    if (fileType == 'bmp' || fileType == 'jpg' || fileType == 'jpeg' || fileType == 'png' || fileType == 'gif') {
                        // var imgSrc = URL.createObjectURL(f);
                        preview = '<img class="easy_upload_img" src="' + data[i].split("~")[0] + '" />';
                    ...
                    $li += '<li class="easy_upload_queue_item" data-index="callbackImg' + i +'">';
                    //图片
                    $li += option.showPreview ? '<div class="easy_upload_preview queue_item-section">' + preview + '</div>' : '';
                    //名称&进度条
                    $li += '<div class="easy_upload_file1 queue_item-section">';
                    $li += '<p class="easy_upload_filename">'+ data[i].split("~")[1] +'</p>';
                    $li += '</div>';
                    //文件大小&进度
                    $li += '<div class="easy_upload_file2 queue_item-section">';
                    $li += '<p class="easy_upload_fiesize">已上传</p>';
                    $li += '</div>';
                    //上传状态
                    $li += '<div class="easy_upload_status queue_item-section">';
                    // $li += file.allow ? sHtml : '<p class="status status6">文件不允许</p>';
                    $li += '</div>';
                    //按钮
                    $li += '<div class="easy_upload_btn queue_item-section">';
                    $li += '<p class="easy_upload_delbtn btn noselect">删除</p>';
                    $li += '</div>';
                }
                $(".easy_upload_queue").append($li);
            }
            this.bindQueue();
          this.bindHead();
        },
        ...
        bindQueue: function () {
          ...
          $('.queue_check').off('click').click(function () {
            ...
            if (checkedAll) {
              $(hItem).attr('data-checked', 'yes');
              $(hItem).attr('checked', true);
            } else {
              $(hItem).attr('data-checked', 'no');
              $(hItem).attr('checked', false);
            }
          });
        ...
        _renderFile: function (fileArr, target) {
          ...
          function render(file) {
            ...
            $html += option.multi ? '<input type="checkbox" class="easyUploadIcon noselect queue_check queue_check_allow_'+ file.allow +'" data-checked="no" data-up="1"/>' : '';
            $html += '</div>';
            if (option.multi) {
              $(queueUl).append($html);
            } else {
        ...
        _handleCheck: function (opt) {
          if (opt.type=='all') {
            ...
            if (opt.check=='yes') {
              //head 的checkbox
              $(opt.target).attr('data-checked', 'yes');
              //子 checkbox

              for (var i=0; i<qItems.length; i++) {
                $(qItems[i]).attr('data-checked', 'yes');
                $(qItems[i]).attr('checked', true);
              }
            } else {
                $(opt.target).attr('data-checked', 'no');
                for (var i=0; i<qItems.length; i++) {
                    $(qItems[i]).attr('data-checked', 'no');
                    $(qItems[i]).attr('checked', false);
                }

              // $(opt.target).attr('checked', false);
            }
          } else {
            if (opt.check=='yes') {
              $(opt.target).attr('data-checked', 'yes');
              $(opt.target).attr('checked', true);
            } else {
              $(opt.target).attr('data-checked', 'no');
              $(opt.target).attr('checked', false);
            }
        ...
        _deleFiles: function(arr,target) {
          ...
          arr.forEach(function(item){
            ...
            var filename = $(target).find(`.easy_upload_queue_item[data-index=${item}]`).find(".easy_upload_filename").html();
            if (option.multi) dele(item);
            var qItem = _this._findEle(item, target);
            if (qItem.upStatus=='2') deleAllowFiles(item);
            // console.log(qItem.upStatus);
            if(null==qItem.upStatus||4==qItem.upStatus){
                option.deleteFunc && option.deleteFunc(filename);
            }
          });
          // console.log($(target).find('.easyUploadIcon').parent());
            // $(target).find('.easyUploadIcon').attr("checked",false);
